...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 mb-6 overflow-hidden">
    <div class="flex border-b border-slate-200">
        ...
        <a href="{{ route('laporan.yearly') }}" class="px-6 py-4 font-medium text-sm border-b-2 border-orange-600 text-orange-600">
            Tahunan
        </a>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
    <form method="GET" action="{{ route('laporan.yearly') }}" class="flex flex-col md:flex-row gap-4 items-end">
        <div class="flex-1">
            ...
            <select name="year" 
                ...
                @for($y = date('Y'); $y >= 2020; $y--)
                    <option value="{{ $y }}-01-01" {{ $year->year == $y ? 'selected' : '' }}>{{ $y }}</option>
                @endfor
            </select>
        </div>
        <div class="flex gap-2">
            ...
            <a href="{{ route('laporan.yearly') }}" class="px-6 py-2 bg-slate-200 text-gray-900 rounded-lg hover:bg-slate-300 transition-colors font-medium">
                Reset
            </a>
        </div>
    </form>
    ...
    <div class="mt-4 pt-4 border-t border-slate-200">
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('laporan.yearly.export-pdf', ['year' => $year->format('Y-m-d')]) }}" 
            ...
            <a href="{{ route('laporan.yearly.export-csv', ['year' => $year->format('Y-m-d')]) }}" 
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <p class="text-xs text-gray-500">Tahun {{ $year->year }}</p>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
...
@if($jumlahTransaksi > 0)
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
    <h3 class="text-lg font-semibold text-gray-900 mb-6">Grafik Penjualan Bulanan - {{ $year->year }}</h3>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-8">
    <div class="p-6 border-b border-slate-200">
        <h3 class="text-lg font-semibold text-gray-900">Statistik Per Kuartal</h3>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-slate-200">
        ...
        <div class="p-6 text-center">
            <p class="text-sm font-medium text-gray-600 mb-3">{{ $stat['quarter'] }}</p>
            <p class="text-2xl font-bold text-orange-600 mb-2">{{ $stat['count'] }}</p>
            <p class="text-xs text-gray-500 mb-3">transaksi</p>
            <p class="text-lg font-bold text-gray-900">Rp{{ number_format($stat['total'], 0, ',', '.') }}</p>
        </div>
        @endforeach
    </div>
</div>
            ...
            <tbody class="divide-y divide-slate-200">
                @foreach($monthlyStats as $stat)
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $stat['month'] }}</td>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
    ...
    <p class="text-gray-600 mb-4">Belum ada transaksi di tahun {{ $year->year }}</p>

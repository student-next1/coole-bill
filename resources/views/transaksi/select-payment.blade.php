...
<div class="max-w-2xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        <!-- Payment Card -->
        <a href="{{ route('transaksi.select-card', ['method' => 'kartu_id']) }}"
           class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:border-orange-300 hover:shadow-md transition-all duration-200 text-center cursor-pointer group">
            ...
            <h4 class="font-semibold text-gray-900 mb-1">Kartu ID</h4>
            <p class="text-xs text-gray-600">Scan atau cari kartu ID</p>
        </a>

        <!-- Cash -->
        <form action="{{ route('transaksi.select-card', ['method' => 'tunai']) }}" method="GET" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:border-orange-300 hover:shadow-md transition-all duration-200">
